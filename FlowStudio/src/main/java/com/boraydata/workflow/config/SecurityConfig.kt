...
import com.boraydata.workflow.entity.AuthorizeVoter
import com.boraydata.flowauth.enums.ErrorCode
import com.boraydata.flowauth.utils.JsonUtils
import com.boraydata.workflow.filter.AuthenticationTokenFilter
...
//    }
    ...
    override fun configure(http: HttpSecurity) {
        ...
        http
            ...
            .authorizeRequests()
            .antMatchers(
                HttpMethod.POST, *postIgnores.split(DEFAULT_PATTERN_SEPERATOR).  // /sms/send/**,/file/**
                map { it.trim() }.toTypedArray()
            ).permitAll()
            .antMatchers(HttpMethod.GET, *getIgnores.split(DEFAULT_PATTERN_SEPERATOR).map { it.trim() }.toTypedArray())
            .permitAll()
            .antMatchers(*allIgnores.split(DEFAULT_PATTERN_SEPERATOR).map { it.trim() }.toTypedArray()).permitAll()
            .antMatchers("/login/**").permitAll()
            .antMatchers("/login").permitAll()
            .and()
            .formLogin().loginPage("/login")
//            .successHandler(successHandlerService) // 成功requestMatchers
            .loginProcessingUrl("/login")
            .failureHandler { _, response, exception ->
                when (exception) {
                    is BadCredentialsException ->
                        response.writer.write(JsonUtils.toJsonStr(fail(ErrorCode.LOGIN_TYPE_ERROR.msg)))
                    is InternalAuthenticationServiceException -> // 这个是什么异常，这个是userservice获取用户时，如果返回空的时候就会捕捉到
